...
# --- COMPLETE SECURITY GUARD (WELCOME, PROFANITY, LINKS, & STRICT MEDIA FILTER) ---
async def security_guard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 1. Rose-Style Welcome Greeter (Triggers when new members join)
    if message.new_chat_members:
        for member in message.new_chat_members:
            if member.id == context.bot.id:
                continue
            
            welcome_text = f"Hey {member.first_name}, welcome to the group! Please verify here to get started."
            sent_msg = await message.reply_text(welcome_text)
            
            # Automatically delete the welcome greeting after 5 seconds (just like Rose)
    ...
    # Trigger deletion if any rule (including raw GIFs/media) is violated
...
def main() -> None:
    ...
    # Comprehensive Protection Handler (Handles welcomes, auto-deletes, profanity, links, and GIFs/media)

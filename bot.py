...
# ============================================================
# MEDIA SETTINGS (TUNED FOR INSTANT SPEED)
# ============================================================

# Checked frames count per video/GIF (kept low for instant execution)
MEDIA_FRAME_COUNT = 4

# Maximum extraction FPS
MAX_FPS = 1

# Smaller width = faster processing on Render CPU
FRAME_WIDTH = 320
...
def nudenet_scan(image_bytes: bytes):
    ...
    try:
        ...
        # Using mode='fast' for rapid inference execution
...
async def scan_frames_parallel(frame_files):
    ...
    try:
        for completed_task in asyncio.as_completed(tasks):
            ...
            if result:
                # Cancel all remaining frame checks instantly upon finding NSFW content
                for task in tasks:
                    if not task.done():
                        task.cancel()

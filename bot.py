...
# Stores user IDs that have successfully completed verification
verified_users = set()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Check if the user came via verification deep link (e.g. /start verify)
    if context.args and context.args[0] == "verify":
        if user_id:
            # Check if they were already verified before tapping again
...
# --- COMPLETE SECURITY GUARD (WELCOME CHECK, PROFANITY, LINKS, & STRICT MEDIA FILTER) ---
async def security_guard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 1. Custom Welcome Greeter with Name and Exact Verification Phrasing
    if message.new_chat_members:
        for member in message.new_chat_members:
            if member.id == context.bot.id:
                continue
            
            # Check if this user has already completed verification previously
            if member.id in verified_users:
                ...
                keyboard = [[InlineKeyboardButton("Verify here you are a human", url=f"https://t.me/{bot_username}?start=verify")]]
                reply_markup = InlineKeyboardMarkup(keyboard)
            
            sent_msg = await message.reply_text(welcome_text, reply_markup=reply_markup)
            
            # Automatically delete the greeting after 15 seconds to keep chat clean

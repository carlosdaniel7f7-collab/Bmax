...
async def check_animation(message):
    ...
    try:
        ...
        # Telegram animations are commonly delivered
        # as MP4 files. Do not try to open the MP4
        # using Pillow.
        #
        # Instead, scan Telegram's animation thumbnail.

        if not message.animation.thumbnail:

            logger.warning(
                "Animation has no thumbnail; "
                "skipping animation scan."
            )
        ...
        if result and is_explicit(result):
            ...
            logger.info(
                "NSFW animation detected from thumbnail."
            )

            return True, result

        logger.info(
            "Animation scanned successfully; "

...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message:
        await update.message.reply_text(help_text, reply_markup=reply_markup, parse_motion="Markdown")
    elif update.callback_query:
...
async def security_guard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 1. Custom Welcome Greeter with exact requested phrasing
    if message.new_chat_members:
        for member in message.new_chat_members:
            if member.id == context.bot.id:
                continue
            
            welcome_text = f"Hello {member.first_name}! Please click below to verify you are human."
            keyboard = [[InlineKeyboardButton("Please click below to verify you are human", url=f"https://t.me/{bot_username}?start=verify")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            sent_msg = await message.reply_text(welcome_text, reply_markup=reply_markup)
            
            # Automatically delete the welcome greeting after 10 seconds to keep chat clean

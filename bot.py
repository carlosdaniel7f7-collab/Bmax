...
# Setup a tiny web server so Render's free tier stays happy
app = Flask('')

@app.route('/')
def home():
    return "Bmax Bot is active and guarding @Anime7p7!"
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "I automatically delete unauthorized links, 18+ spam, GIFs, stickers, and profanity to keep your community clean.\n\n"
...
# Comprehensive cleaner for Text, GIFs, Stickers, Photos, and Videos
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Extract text from message body or caption (covers photos, videos, animations/GIFs)
    if message.text:
    ...
    # Check if the message contains restricted media types (Stickers, Animation/GIFs)
...
def main():
    # Start the web server in the background so Render stays happy
    keep_alive()
    ...
    # Listen to all kinds of content updates (text, photos, videos, stickers, animations)

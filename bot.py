...
try:
    ...
    raise RuntimeError(
        f"Could not initialize Google Vision: {e}"
    )
...
def google_safe_search(image_bytes: bytes):
    ...
    try:
        ...
        result = {
            ...
            "spoof": int(safe.spoof),
        }

        logger.info(
            "SafeSearch - adult=%s, racy=%s",
            likelihood_name(
                result["adult"]
            ),
            likelihood_name(
...
def is_explicit(result):
    ...
    # ADULT:
    #
    # LIKELY or VERY_LIKELY = delete
    #
    # RACY:
    #
    # VERY_LIKELY = delete
    #
    # This keeps normal images from being too aggressively
    # removed while still targeting obvious sexual content.
    # --------------------------------------------------------
...
async def check_animation(message):
    ...
    try:
        ...
        # Telegram animations often have a thumbnail.
        # We scan that first.
        ...
        # Try downloading the animation itself.
        telegram_file = (
...
async def start(
    ...
    welcome_text = (
        ...
        "• Google SafeSearch NSFW Detection\n"
...
async def features_command(
    ...
    features_text = (
        ...
        "🖼️ Google SafeSearch Photo Detection\n"
...
async def security_guard(
    ...
    if message.photo:
        ...
        if is_nsfw:
            ...
            await delete_message_safely(
                message,
                "NSFW photo detected by Google SafeSearch"
            )
    ...
    if message.animation:
        ...
        if is_nsfw:
            ...
            await delete_message_safely(
                message,
                "NSFW animation detected by Google SafeSearch"
            )
    ...
    if message.video:
        ...
        if is_nsfw:
            ...
            await delete_message_safely(
                message,
                "NSFW video thumbnail detected by Google SafeSearch"
            )

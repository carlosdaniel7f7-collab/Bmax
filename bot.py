...
# Maximum frames for long media.
MAX_MEDIA_FRAMES = 24

# Minimum frames for short media.
MIN_MEDIA_FRAMES = 8
...
# Number of frames scanned concurrently.
MEDIA_SCAN_WORKERS = 4
...
def calculate_frame_count(
    ...
    # Short GIFs/videos get fewer frames.
    if duration <= 2:
...
async def extract_and_scan_media(
    ...
    try:
        ...
        if duration:
            ...
            filter_value = (
                f"fps={fps:.4f},scale=640:-2"
            )

        else:

            filter_value = (
                f"fps={MAX_FPS},scale=640:-2"
            )
        ...
        logger.info(
            "Starting parallel scan of %d frames...",
            len(frame_files)
        )

        is_nsfw, result = (
            await scan_frames_parallel(
                frame_files
            )
        )
...
async def status_command(
    ...
    status_text = (
        ...
        "• Fast GIF Scanner: Active\n"
        "• GIF Document Scanner: Active\n"
        "• Fast Video Scanner: Active\n"
        "• Parallel Media Scanning: Active\n"
        "• Verification: Active"
    )
...
async def features_command(
    ...
    features_text = (
        ...
        "🎞️ Fast Multi-Frame GIF Detection\n"
        "📁 GIF Document Detection\n"
        "🎥 Fast Multi-Frame Video Detection\n"
